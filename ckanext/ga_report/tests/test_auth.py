...
class TestAuth:
    @classmethod
    def setup_class(cls):
        if not os.path.exists("token.dat") or not os.path.exists(
            "credentials.json"
        ):
            print("*" * 60)
            print(
                "Tests may not run without first having run the auth process"
            )
            print("*" * 60)
    ...
    def test_get_profile(self):
        ...
        assert (
            profile is not None
        ), "Unable to find a profile given configured UA id and user details"
