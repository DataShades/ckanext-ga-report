    ...
    if dataset_match:
        ...
        if dataset:
            owner_org = (
                ...
                .filter(group.Group.id == dataset.owner_org)
                .first()
            )
            if owner_org:
...
def update_sitewide_stats(period_name, stat_name, data, period_complete_day):
    for k, v in data.iteritems():
        item = (
            ...
            .filter(GA_Stat.stat_name == stat_name)
            .first()
        )
        if item:
...
def pre_update_url_stats(period_name):
    ...
    q = model.Session.query(GA_Url).filter(GA_Url.period_name == "All")
...
def post_update_url_stats():
    ...
    for key in views.keys():
        ...
        uuidregex = re.compile(
            "\/data/dataset\/[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}"
        )
    ...
    for url in url_data:
        ...
        if package:
            ...
            values = {
                ...
                + int(old_pageviews),
                "visits": sum([int(e.visits or 0) for e in entries])
                + int(old_visits),
                "department_id": publisher,
                "package_id": package,
            }
    ...
    for url, data in data.iteritems():
        for entry in data:
            ...
            item = (
                ...
                .filter(GA_ReferralStat.url == url)
                .first()
            )
            if item:
...
def update_publisher_stats(period_name):
    ...
    for publisher in publishers:
        ...
        item = (
            model.Session.query(GA_Publisher)
            .filter(GA_Publisher.period_name == period_name)
            .filter(GA_Publisher.publisher_name == publisher.name)
            .first()
        )
        if item:
            item.views = views
...
def get_score_for_dataset(dataset_name):
    ...
    for period_name in period_names:
        ...
        entry = (
            model.Session.query(GA_Url)
            .filter(GA_Url.period_name == period_name)
            .filter(GA_Url.package_id == dataset_name)
            .first()
        )
        # score
        if entry:
