...
def _get_package_and_publisher(url):
    ...
    if dataset_match:
        ...
        if not dataset:
            ...
            if dataset_rev:
                dataset = model.Session.query(
                    model.Package
                ).filter(model.Package.id == dataset_rev.id).first()
        if dataset:
            owner_org = model.Session.query(
                group.Group
            ).filter(group.Group.id == dataset.owner_org).first()
            if owner_org:
...
def update_sitewide_stats(period_name, stat_name, data, period_complete_day):
    for k, v in data.iteritems():
        item = model.Session.query(GA_Stat).filter(
            GA_Stat.period_name == period_name
        ).filter(GA_Stat.key == k).filter(GA_Stat.stat_name == stat_name
                                          ).first()
        if item:
...
def pre_update_url_stats(period_name):
    q = model.Session.query(GA_Url).filter(GA_Url.period_name == period_name)
...
def post_update_url_stats():
    ...
    query = """select url, pageviews::int, visits::int
               from ga_url
               where url not in
               (select url from ga_url where period_name ='All')"""
...
def update_url_stats(period_name, period_complete_day, data):
    ...
    for url in url_data:
        ...
        if progress_count % 100 == 0:
            log.debug('.. %d/%d done so far', progress_count, progress_total)
        item = model.Session.query(GA_Url).filter(
            GA_Url.period_name == period_name
        ...
        if package:
            ...
            old = model.Session.query(GA_Url).filter(
            ...
            entries = model.Session.query(GA_Url).filter(
                GA_Url.period_name != 'All'
            ).filter(GA_Url.url == url).all()
            values = {
                'id': make_uuid(),
                ...
                'period_complete_day': 0,
                'url': url,
                'pageviews': sum([int(e.pageviews)
                                  for e in entries]) + int(old_pageviews),
                'visits': sum([int(e.visits or 0)
                               for e in entries]) + int(old_visits),
                'department_id': publisher,
                'package_id': package
            }
...
def update_social(period_name, data):
    # Clean up first.
    model.Session.query(GA_ReferralStat).filter(
    ...
    for url, data in data.iteritems():
        for entry in data:
            ...
            item = model.Session.query(GA_ReferralStat).filter(
                GA_ReferralStat.period_name == period_name
...
def update_publisher_stats(period_name):
    ...
    publishers = model.Session.query(
        model.Group
    ).filter(model.Group.type == 'organization'
             ).filter(model.Group.state == 'active').all()
    for publisher in publishers:
        views, visits, subpub = update_publisher(
            period_name, publisher, publisher.name
        )
        parent, parents = '', publisher.get_parent_groups(type='organization')
        if parents:
            parent = parents[0].name
        item = model.Session.query(GA_Publisher).filter(
            GA_Publisher.period_name == period_name
...
def update_publisher(period_name, pub, part=''):
    ...
    for publisher in go_down_tree(pub):
        subpub = subpub + 1
        items = model.Session.query(GA_Url).filter(
            GA_Url.period_name == period_name
...
def get_top_level():
    ...
    return model.Session.query(model.Group).outerjoin(
        model.Member, model.Member.table_id == model.Group.id
        and model.Member.table_name == 'group'
        and model.Member.state == 'active'
    ).filter(model.Member.id == None
             ).filter(model.Group.type == 'organization').order_by(
                 model.Group.name
             ).all()
...
def go_down_tree(publisher):
    '''
    Provided with a publisher object,
    it walks down the hierarchy and yields each publisher,
...
def get_score_for_dataset(dataset_name):
    ...
    period_names = [
        '%s-%02d' % (last_month.year, last_month.month),
    ...
    for period_name in period_names:
        ...
        entry = model.Session.query(GA_Url).filter(
            GA_Url.period_name == period_name
