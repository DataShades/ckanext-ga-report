...
class DownloadAnalytics(object):
    ...
    def __init__(
        ...
        self.period = tk.config["ga-report.period"]
        ...
        if self.period == "monthly":
            ...
            while True:
                ...
                if first_of_the_month == first_of_this_month:
                    ...
                    in_the_next_month = (
                        first_of_the_month + datetime.timedelta(40)
                    )
    ...
    def download_and_store(self, periods):
        for period_name, period_complete_day, start_date, end_date in periods:
            ...
            if not self.skip_url_stats:
                ...
                self.store(
                    period_name, period_complete_day, data,
                )
    ...
    def sitewide_stats(self, period_name, period_complete_day):
        ...
        for f in funcs:
            ...
            getattr(self, f)(
                start_date, end_date, period_name, period_complete_day
            )
    ...
    def _totals_stats(
        ...
        ga_model.update_sitewide_stats(
            period_name,
            "Totals",
            {"Total page views": result_data[0][0]},
            period_complete_day,
        )
        ...
        try:
            ...
            args[
                "metrics"
            ] = "ga:pageviewsPerSession,ga:avgSessionDuration,ga:percentNewSessions,ga:sessions,ga:users"
            args["alt"] = "json"
    ...
    def _download_stats(
        ...
        def process_result_data(result_data, cached=False):
            ...
            for result in result_data:
                ...
                if cached:
                    r = q.filter(
                        model.Resource.cache_url.like("%s%%" % url)
                    ).first()
                else:
                ...
                if re.search("(?:\/resource\/)(.*)(?:\/download\/)", url):
                    ...
                    r = q.filter(
                        model.Resource.id == resource_id.group(1)
                    ).first()
                    if not r:
                ...
                if package_name:
                    ...
                    data[package_name] = data.get(package_name, 0) + int(
                        result[1]
                    )
                else:
            ...
            if resources_not_matched:
                log.debug(
                    "Could not match %i or %i resource URLs to datasets. e.g. %r",
                    len(resources_not_matched),
                    progress_total,
                    resources_not_matched[:3],
                )

        log.info(
            "Associating downloads of resource URLs with their respective datasets"
        )
