...
class DownloadAnalytics(object):
    ...
    def specific_month(self, date):
        ...
        last_of_this_month = datetime.datetime(
    ...
    def latest(self):
        if self.period == 'monthly':
            ...
            periods = ((
                now.strftime(FORMAT_MONTH), now.day, first_of_this_month, now
            ), )
        else:
            raise NotImplementedError
        self.download_and_store(periods)

    def for_date(self, since_date):
    ...
    def download_and_store(self, periods):
        for period_name, period_complete_day, start_date, end_date in periods:
            ...
            if self.delete_first:
                log.info(
                    'Deleting existing Analytics for this period "%s"',
                    period_name
                )
            ...
            if not self.skip_url_stats:
                ...
                log.info(
                    'Storing dataset views (%i rows)', len(data.get('url'))
                )
                self.store(
                    period_name,
                    period_complete_day,
                ...
                log.info(
                    'Storing publisher views (%i rows)', len(data.get('url'))
                )
                self.store(
                    period_name,
                    period_complete_day,
    ...
    def update_social_info(self, period_name, start_date, end_date):
        ...
        try:
            # Because of issues of invalid responses, we are going to
            # make these requests ourselves.
            args = dict(
    ...
    def download(self, start_date, end_date, path=None):
        ...
        if results['totalResults'] > 0:
            for entry in results.get('rows', []):
                ...
                # url = _normalize_url('http:/' + loc) # strips off domain e.g.
    ...
    def store(self, period_name, period_complete_day, data):
        if 'url' in data:
            ga_model.update_url_stats(
                period_name, period_complete_day, data['url']
            )
    ...
    def sitewide_stats(self, period_name, period_complete_day):
        ...
        funcs = [
            '_totals_stats', '_social_stats', '_os_stats', '_locale_stats',
            '_browser_stats', '_mobile_stats', '_download_stats'
        ]
        for f in funcs:
            log.info('Downloading analytics for %s' % f.split('_')[1])
            getattr(self,
                    f)(start_date, end_date, period_name, period_complete_day)
    ...
    def _get_json(self, params, prev_fail=False):
        ...
        if not ga_token_filepath:
            print(
                'ERROR: In the CKAN config you need'
                ' to specify the filepath of the '
                'Google Analytics token file under'
                ' key: googleanalytics.token.filepath'
            )
            return
        try:
        ...
        def process_result_data(result_data, cached=False):
            ...
            for result in result_data:
                ...
                if progress_count % 100 == 0:
                    log.debug(
                        '.. %d/%d done so far', progress_count, progress_total
                    )

                url = urllib.unquote(result[0].strip())

                # Get package id associated with the resource that has this
                # URL.
                q = model.Session.query(model.Resource)
                if cached:
                    r = q.filter(model.Resource.cache_url.like("%s%%" % url)
                                 ).first()
                else:
                ...
                if re.search('(?:\/resource\/)(.*)(?:\/download\/)', url):
                    ...
                    if not r:
                        ...
                        if filename:
                            sql = (
                                "SELECT distinct id FROM public.resource t "
                                "WHERE url ilike '%{0}%' "
                                "UNION SELECT distinct id "
                                "FROM public.resource_revision t "
                                "WHERE url ilike '%{0}%' "
                                "UNION SELECT distinct id "
                                "FROM public.resource t "
                                "WHERE replace(url,'-','') ilike '%{0}%' "
                                "UNION SELECT distinct id "
                                "FROM public.resource_revision t "
                                "WHERE replace(url,'-','') ilike '%{0}%' "
                ...
                package_name = model.Session.query(
                    model.Package
                ).filter(model.Package.id == r.package_id
                         ).first().name if r else ""
            ...
            if resources_not_matched:
                log.debug(
                    'Could not match %i or %i '
                    'resource URLs to datasets. e.g. %r',
                    len(resources_not_matched), progress_total,
                    resources_not_matched[:3]
                )
    ...
    def _mobile_stats(
        ...
        if results is not None:
            for result in result_data:
                data[result[0]] = data.get(result[0], 0) + int(result[2])
        ga_model.update_sitewide_stats(
            period_name, "Mobile brands", data, period_complete_day
        )
        ...
        for result in result_data:
            data[result[1]] = data.get(result[1], 0) + int(result[2])
        ga_model.update_sitewide_stats(
            period_name, "Mobile devices", data, period_complete_day
        )
