...
import sqlalchemy
from ckan.common import g, c, response, request
...
def _month_details(cls, stat_key=None):
    ...
    q = model.Session.query(
        cls.period_name, cls.period_complete_day
...
class GaReport(BaseController):
    def csv(self, month):
        ...
        q = model.Session.query(GA_Stat).filter(
            GA_Stat.stat_name != 'Downloads'
        )
        if month != 'all':
        ...
        for entry in entries:
            writer.writerow([
                entry.period_name.encode('utf-8'),
    ...
    def index(self):
        ...
        sparkline_query = model.Session.query(GA_Stat).filter(
            GA_Stat.stat_name == 'Totals'
        ...
        for entry in q.all():
            c.social_referrers.append((
                shorten_name(entry.url), fill_out_url(entry.url), entry.source,
                entry.count
            ))
        ...
        for k, v in keys.iteritems():
            q = model.Session.query(GA_Stat).filter(GA_Stat.stat_name == k
                                                    ).order_by(
                                                        GA_Stat.period_name
                                                    )
            ...
            graph_query = model.Session.query(GA_Stat).filter(
                GA_Stat.stat_name == k
            ).order_by(GA_Stat.period_name)
            graph_dict = {}
            ...
            if k == 'Social sources':
                total = sum([
                    x for n, x, _ in c.global_totals if n == 'Total visits'
                ])
            else:
...
class GaDatasetReport(BaseController):
    ...
    def publisher_csv(self, month):
        ...
        writer = csv.writer(response)
        writer.writerow([
            "Publisher Title", "Publisher Name", "Views", "Visits",
            "Period Name"
        ])
        ...
        for publisher, view, visit in top_publishers:
            writer.writerow([
                publisher.title.encode('utf-8'),
                publisher.name.encode('utf-8'), view, visit, month
            ])
    ...
    def dataset_csv(self, id='all', month='all'):
        ...
        writer = csv.writer(response)
        writer.writerow([
            "Dataset Title", "Dataset Name", "Views", "Visits",
            "Resource downloads", "Dataset formats", "Period Name"
        ])

        for package, view, visit, downloads, formats in packages:
            writer.writerow([
                package.title.encode('utf-8'),
                package.name.encode('utf-8'), view, visit, downloads, formats,
                month
            ])
    ...
    def _get_packages(self, publisher=None, month='', count=-1):
        ...
        q = model.Session.query(
            GA_Url, model.Package
        ).filter(model.Package.name == GA_Url.package_id).filter(
        ...
        for entry, package in entries:
            if package:
                ...
                if have_download_data:
                    dls = model.Session.query(GA_Stat).filter(
                        GA_Stat.stat_name == 'Downloads'
    ...
    def read_publisher(self, id):
        '''
        Lists the most popular datasets for a publisher
        (or across all publishers)
        '''
        ...
        q = model.Session.query(GA_Url).filter(
            GA_Url.url == '/publisher/%s' % c.publisher_name
        )
        entry = q.filter(GA_Url.period_name == c.month).first()
        c.publisher_page_views = entry.pageviews if entry else 0

        c.top_packages = self._get_packages(
            publisher=c.publisher, count=100, month=c.month
        )

        # Graph query
        top_packages_all_time = self._get_packages(
            publisher=c.publisher, count=20, month='All'
        )
        top_package_names = [x[0].name for x in top_packages_all_time]
        graph_query = model.Session.query(
            GA_Url, model.Package
        ).filter(model.Package.name == GA_Url.package_id).filter(
            GA_Url.url.like('%/dataset/%')
    ...
    for series in raw_data:
        for point in series['data']:
            ...
            if not (series in data) and percentage > THRESHOLD:
                data.append(series)
            point['y'] = percentage
    others = [item for item in raw_data if not (item in data)]
    if len(others):
        data_other = []
...
def _get_top_publishers_graph(limit=20):
    ...
    q = model.Session.query(
        GA_Url.department_id, GA_Url.period_name,
        func.sum(cast(GA_Url.pageviews, sqlalchemy.types.INT))
    ).filter(GA_Url.department_id.in_(department_ids)).filter(
        GA_Url.url.like('%/dataset/%')
    ).filter(GA_Url.package_id != '').group_by(
        GA_Url.department_id, GA_Url.period_name
    )
    graph_dict = {}
...
def _get_publishers():
    ...
    '''
    publishers = []
    for pub in model.Session.query(
        model.Group
    ).filter(model.Group.type == 'organization'
             ).filter(model.Group.state == 'active').order_by(
                 model.Group.name
             ):
