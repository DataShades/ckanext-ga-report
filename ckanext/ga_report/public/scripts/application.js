...
(function($) {
  $(document).ready(function() {
    ...
    CKAN.Utils.setupOrganizationUserAutocomplete(
      $("input.autocomplete-organization-user")
    );
    ...
    if (isDatasetNew || isDatasetResourceEdit) {
      ...
      var CollectionOfResources = Backbone.Collection.extend({
    ...
    if (window.openid && openid.signin) {
      ...
      openid.signin = function(arg) {
        $.get(CKAN.SITE_URL + "/user/set_lang/" + CKAN.LANG, function() {
          openid._signin(arg);
        });
      };
    }
...
jQuery.fn.truncate = function(max, suffix) {
  return this.each(function() {
    ...
    if (isTruncated) {
      element.html(
        element.data(
          "truncate:" + (max === "expand" ? "original" : "truncated")
        )
      );
      return;
    }
...
CKAN.View.UrlEditor = Backbone.View.extend({
  initialize: function() {
    ...
    this.regexToDelete = [new RegExp("^-*", "g"), new RegExp("-*$", "g")];
  ...
  titleToSlug: function(title) {
    var slug = title;
    $.each(this.regexToHyphen, function(idx, regex) {
      slug = slug.replace(regex, "-");
    });
    $.each(this.regexToDelete, function(idx, regex) {
      slug = slug.replace(regex, "");
    });
    ...
    if (slug.length < 2) {
      this.validMsg.html(
        '<span style="font-weight: bold; color: #444;">' +
          CKAN.Strings.urlIsTooShort +
          "</span>"
      );
    } else if (slug == this.originalUrl) {
      this.validMsg.html(
        '<span style="font-weight: bold; color: #000;">' +
          CKAN.Strings.urlIsUnchanged +
          "</span>"
      );
    } else {
      this.validMsg.html(
        '<span style="color: #777;">' + CKAN.Strings.checking + "</span>"
      );
      var self = this;
    ...
    if (data.valid) {
      this.validMsg.html(
        '<span style="font-weight: bold; color: #0c0">' +
          CKAN.Strings.urlIsAvailable +
          "</span>"
      );
    } else {
      this.validMsg.html(
        '<span style="font-weight: bold; color: #c00">' +
          CKAN.Strings.urlIsNotAvailable +
          "</span>"
      );
    }
  }
...
CKAN.View.Resource = Backbone.View.extend({
  ...
  render: function() {
    ...
    this.descriptionBox = this.table.find(
      "textarea.js-resource-edit-description"
    );
  ...
  updateIcon: function() {
    ...
    self.updateIconTimer = setTimeout(function() {
      ...
      $.getJSON(
        CKAN.SITE_URL +
          "/api/2/util/resource/format_icon?format=" +
          encodeURIComponent(self.formatBox.val()),
        function(data) {
  ...
  openMyPanel: function(e) {
    ...
    this.li
      .parents("fieldset#resources")
      .find(".resource-list > li")
      .removeClass("active");
    panel.show();
    this.table.show();
  ...
  askToDelete: function(e) {
    e.preventDefault();
    var confirmMessage = CKAN.Strings.deleteThisResourceQuestion.replace(
      "%name%",
      this.name()
    );
  ...
  addDynamicExtra: function(key, value) {
    // Create elements
    var dynamicExtra = $(
      $.tmpl(CKAN.Templates.resourceExtra, {
        num: this.options.position,
        key: key,
        value: value
      })
...
CKAN.View.ResourceAddUpload = Backbone.View.extend({
  ...
  setupFileUpload: function() {
    ...
    this.el.find(".fileupload").fileupload({
      ...
      send: function(e, data) {
        self.setMessage(
          CKAN.Strings.uploadingFile +
            ' <img src="http://assets.okfn.org/images/icons/ajaxload-circle.gif" class="spinner" />'
        );
      },
  ...
  makeUploadKey: function(fileName) {
    ...
    var str = this.ISODateString(now)
      .replace(":", "")
      .replace(":", "");
  ...
  updateFormData: function(key) {
    var self = this;
    self.setMessage(
      CKAN.Strings.checkingUploadPermissions +
        ' <img src="http://assets.okfn.org/images/icons/ajaxload-circle.gif" class="spinner" />'
    );
        ...
        newResource.set(
          ...
          },
          {
            error: function(model, error) {
              var msg =
                "Filed uploaded OK but error adding resource: " + error + ".";
              msg +=
                "You may need to create a resource directly. Uploaded file at: " +
                data._location;
...
CKAN.Utils = (function($, my) {
  ...
  my.setupPackageAutocomplete = function(elements) {
    elements.autocomplete({
      ...
      select: function(event, ui) {
        ...
        var new_name =
          split[1] + "__" + (parseInt(split[2], 10) + 1) + "__" + split[3];
        ...
        $new.append('<i class="fa fa-plus-sign"></i> ');
      ...
      select: function(event, ui) {
        ...
        parent_dd.before(
          '<input type="hidden" name="' +
            ...
            '">' +
            '<input type="hidden" name="' +
            old_name.replace("__name", "__capacity") +
            '" value="editor">' +
            "<dd>" +
            ui.item.label +
            "</dd>"
  ...
  my.setupUserAutocomplete = function(elements) {
    elements.autocomplete({
      ...
      source: function(request, callback) {
        var url =
          CKAN.SITE_URL + "/api/2/util/user/autocomplete?q=" + request.term;
        $.getJSON(url, function(data) {
          $.each(data, function(idx, userobj) {
            var label = userobj.name;
            if (userobj.fullname) {
  ...
  my.relatedSetup = function(form) {
    ...
    function relatedRequest(action, method, data) {
      return $.ajax({
        ...
        error: function(err, txt, w) {
          ...
          addAlert(
            "<strong>Error:</strong> Unable to " + action + " related item"
          );
        }
      });
    }
    ...
    relatedItems.on("mouseenter mouseleave", ".description.truncated", function(
      event
    ) {
  ...
  my.setupGroupAutocomplete = function(elements) {
    elements.autocomplete({
      ...
      source: function(request, callback) {
        var url =
          CKAN.SITE_URL + "/api/2/util/group/autocomplete?q=" + request.term;
        $.getJSON(url, function(data) {
          $.each(data, function(idx, userobj) {
            var label = userobj.name;
            userobj.label = label;
  ...
  my.setupMarkdownEditor = function(markdownEditor) {
    ...
    markdownEditor
      .find("button, div.markdown-preview")
      .live("click", function(e) {
        ...
        if ($target.is(".js-markdown-preview")) {
          ...
          $.post(
            CKAN.SITE_URL + "/api/util/markdown",
            { q: raw_markdown },
            function(data) {
              preview.html(data);
            }
  ...
  my.setupNotesExtract = function() {
    ...
    if (paragraphs.length === 0) {
      ...
      $.each(paragraphs, function(i, para) {
        if (i > 0) {
          remainder.append($(para).remove());
        }
      });
      var finalHeight = remainder.height();
      remainder.height(0);
      notes.find("#notes-toggle").show();
      notes.find("#notes-toggle button").click(function(event) {
  ...
  my.loadPreviewDialog = function(resourceData) {
    ...
    function initializeDataExplorer(dataset) {
      ...
      function updateLink() {
        ...
        embedIframeText.val(
          $.mustache(
            '<iframe frameBorder="0" width="{{width}}" height="{{height}}" src="{{link}}"></iframe>',
            {
              link: link.replace(/"/g, "&quot;"),
              width: width,
              height: height
    ...
    if (resourceData.datastore_active) {
      ...
      var errorMsg =
        CKAN.Strings.errorLoadingPreview + ": " + CKAN.Strings.errorDataStore;
      dataset
        .fetch()
      ...
      var dataset = new recline.Model.Dataset(resourceData);
      var errorMsg =
        CKAN.Strings.errorLoadingPreview + ": " + CKAN.Strings.errorDataProxy;
      dataset
        .fetch()
        .done(function(dataset) {
      ...
      resourceData.formatNormalized in
      {
        "rdf+xml": "",
        "owl+xml": "",
        xml: "",
      ...
      resourceData.formatNormalized in { html: "", htm: "" } ||
    ...
    // images
      ...
      resourceData.formatNormalized in { png: "", jpg: "", gif: "" } ||
      ...
      my.showError({
        title:
          CKAN.Strings.previewNotAvailableForDataType +
          resourceData.formatNormalized,
        message: ""
      });
    }
  };
    ...
    var timer = setTimeout(function error() {
      callback({
        error: {
          title: "Request Error",
          message:
            "Dataproxy server did not respond after " +
            timeout / 1000 +
            " seconds"
        }
      });
